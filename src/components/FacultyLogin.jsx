...
const FacultyLogin = () => {
  ...
  if (showForgotPage) return <FacultyForgetPassword />

  const handleLogin = async e => {
    e.preventDefault()
    try {
      const response = await axios.post('http://localhost:4000/api/faculty/login', { email, password })
      ...
      navigate('/faculty/profile') // redirect to profile
    } catch(err) {
    ...
    <div>
      ...
      <div style={{display:'flex',justifyContent:'center',alignItems:'center',height:'100vh',backgroundColor:'#f4f4f9'}}>
        <div style={{background:'white',padding:'40px',borderRadius:'12px',boxShadow:'0 4px 15px rgba(0,0,0,0.2)',width:'350px',textAlign:'center'}}>
          ...
          <p>
            <Link to="/faculty/register" style={linkStyle}>New User? Register Here</Link>
          </p>
        </div>
      </div>
    </div>
